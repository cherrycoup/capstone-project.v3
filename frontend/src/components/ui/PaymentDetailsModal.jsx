        ...
        <div className="space-y-6 px-6 py-6">
          ...
          <div className="space-y-3 rounded-3xl border border-slate-200 bg-slate-50 p-4">
            {details.fields.map((field) => (
              <div key={field.label} className="flex items-center justify-between gap-4 text-sm text-slate-700">
                <span className="font-medium text-slate-600">{field.label}</span>
                <span className="rounded-full bg-white px-3 py-2 text-slate-900 shadow-sm">{field.value}</span>
              </div>
            ))}
          </div>
